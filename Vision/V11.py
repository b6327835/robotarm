...
while True:
    ...
    # Loop through the sorted objects to draw them with priority
    for priority, (x1, y1, x2, y2, target_x, target_y, conf) in enumerate(detected_objects, start=1):
        ...
        # Display priority and confidence above the dot
        # text = f"Priority: {priority}, Conf: {conf:.1f}%"

        # cv2.putText(frame, text, (target_x + 10, target_y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
        cv2.putText(frame,f"P:{priority},X:{target_x},Y:{target_y}", (target_x - 10, target_y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

    # Show the frame with detected objects, center dots, lines, and text
    cv2.imshow('YOLOv8 Detection with Multiple Objects and Priority', frame)
