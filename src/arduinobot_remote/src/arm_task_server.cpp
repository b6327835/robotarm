...
public:
  explicit ArmTaskServer(const rclcpp::NodeOptions& options = rclcpp::NodeOptions())
    : Node("arm_task_server", options)
  {
    RCLCPP_INFO(get_logger(), "Starting the Arm XYZ Server");
    action_server_ = rclcpp_action::create_server<arduinobot_msgs::action::ArmTask>(
        this, "arm_task_server", std::bind(&ArmTaskServer::goalCallback, this, _1, _2),
...
private:
      ...
      std::shared_ptr<const arduinobot_msgs::action::ArmTask::Goal> goal)
  {
    ...
    RCLCPP_INFO(get_logger(), "Received goal request with coordinates: [%f, %f, %f]",
      ...
      const std::shared_ptr<rclcpp_action::ServerGoalHandle<arduinobot_msgs::action::ArmTask>> goal_handle)
  {
    ...
    std::thread{ std::bind(&ArmTaskServer::execute, this, _1), goal_handle }.detach();
  }
  ...
  void execute(const std::shared_ptr<rclcpp_action::ServerGoalHandle<arduinobot_msgs::action::ArmTask>> goal_handle)
  {
    ...
    if (!arm_within_bounds)
    {
      RCLCPP_WARN(get_logger(),
                  "Target joint position(s) were outside of limits, but will plan and to the limits");
      return;
    }
...
RCLCPP_COMPONENTS_REGISTER_NODE(arduinobot_remote::ArmTaskServer)