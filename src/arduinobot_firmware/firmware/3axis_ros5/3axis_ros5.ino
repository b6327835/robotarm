...
void setup() {
  ...
  if (stepperX && stepperY && stepperZ) {
    ...
    Serial.println("Enter parameters as 'X,Y,Z,MS,LP,MAX,ACC'");
    Serial.println("Example: '0,0,0,16,2,40000,320000'");
  } else {
...
bool parseParameters(String input, float &x, float &y, float &z, float &microstepping, float &leadScrewPitch, int32_t &maxSpeed, int32_t &acceleration) {
  ...
  if (firstComma == -1 || secondComma == -1 || thirdComma == -1 || fourthComma == -1 || 
    ...
    Serial.println("Invalid format. Use X,Y,Z,MS,LP,MAX,ACC");
    return false;
  }
  ...
  acceleration = input.substring(sixthComma + 1).toInt();
...
void loop() {
  if (Serial.available()) {
    ...
    int32_t maxSpeed, acceleration;
    
    if (parseParameters(input, x, y, z, microstepping, leadScrewPitch, maxSpeed, acceleration)) {
      moveToPosition(x, y, z, microstepping, leadScrewPitch, maxSpeed, acceleration);
    }
  }
}
