...
void moveToPosition(float x, float y, float z, float microstepping, float leadScrewPitch, int32_t maxSpeed, int32_t acceleration) {
  ...
  // After movement completes, check Z position for vacuum control
  float rosZ = z * (ROS_MAX_Z/MAX_Z);  // Convert current Z to ROS coordinates
  
  if (rosZ > VACUUM_TRIGGER_HEIGHT) {
    if (!isPicking && !isPlacing) {
      // First time above threshold - activate picking
      ...
      // Second time above threshold - switch to placing
    ...
    // Below threshold - reset states if placing was active
    if (isPlacing) {
      digitalWrite(VACUUM_PLACE_PIN, HIGH); // Turn off placing vacuum
      isPlacing = false;
      Serial.println("All vacuum OFF");
    }
  }
