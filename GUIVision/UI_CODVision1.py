...
class VideoThread(QThread):
    ...
    # targetxy = pyqtSignal([float],[float])
    # global target_gx
    # global target_gy
    def run(self):
        ...
        while True:
            ...
            detected_objects = (
                []
            )  # Store detected objects with their box info and confidence
                ...
                for box in result.boxes:
                    ...
                    if conf > 10:
                        ...
                        detected_objects.append(
                            (x1, y1, x2, y2, self.target_x, self.target_y, conf)
                        )
            ...
            for priority, (
                ...
                self.target_y,
                conf,
            ) in enumerate(detected_objects, start=1):
                # Draw the bounding box
                # cv2.rectangle(cv_img, (x1, y1), (x2, y2), (0, 255, 0), 2)

                # Draw a dot at the center of the object
                # self.target_gx = self.target_x
                # self.target_gy = self.target_y
                cv2.circle(cv_img, (self.target_x, self.target_y), 5, (0, 0, 255), -1)
                cv2.putText(
                    cv_img,
                    f"P:{priority},X:{self.target_x},Y:{self.target_y}",
