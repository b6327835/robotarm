...
class VideoThread(QThread):
    ...
    def run(self): 
        ...
        while True:
            ...
            # Resize cv_img to 300x200 in case it's not the same
            ...
            for result in results:
                ...
                for box in result.boxes:
                    ...
                    if conf > 60:
                        ...
                        detected_objects.append((x1, y1, x2, y2, self.target_x, self.target_y, conf))
            ...
            for priority, (x1, y1, x2, y2, self.target_x, self.target_y, conf) in enumerate(detected_objects, start=1):
                # Draw the bounding box
                # cv2.rectangle(cv_img, (x1, y1), (x2, y2), (0, 255, 0), 2)

                # Draw a dot at the center of the object
                cv2.circle(cv_img, (self.target_x, self.target_y), 5, (0, 0, 255), -1)

                # Draw horizontal and vertical lines across the object center
                # cv2.line(cv_img, (self.target_x, y1), (self.target_x, y2), (0, 255, 255), 1)  # Vertical line
                # cv2.line(cv_img, (x1, self.target_y), (x2, self.target_y), (0, 255, 255), 1)  # Horizontal line

                # Display priority and confidence above the dot
                # text = f"Priority: {priority}, Conf: {conf:.1f}%"

                # cv2.putText(cv_img, text, (self.target_x + 10, self.target_y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
                cv2.putText(cv_img,f"P:{priority},X:{self.target_x},Y:{self.target_y}", (self.target_x - 10, self.target_y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
...
class myclass(Ui_MainWindow):
    ...
    def VISION_GO(self):
        self.Vision_X.setNum(int(self.target_x))
